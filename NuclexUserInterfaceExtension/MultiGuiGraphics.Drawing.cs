...
namespace NuclexUserInterfaceExtension {
  ...
  partial class MultiGuiGraphics {
    ...
    /// <returns>An object that will unset the clipping region upon its destruction.</returns>
    /// <remarks>
    ...
    public void DrawString(string frameName, RectangleF bounds, string text) 
    {
        ...
        {
            this.spriteBatch.DrawString(frame.Texts[index].Font, text, positionText(ref frame.Texts[index], bounds, text), frame.Texts[index].Color);
        }
    }
    ...
    public void DrawCaret(string frameName, RectangleF bounds, string text, int caretIndex) 
    {
        ...
        for (int index = 0; index < frame.Texts.Length; ++index) 
        {
            ...
            this.spriteBatch.DrawString(frame.Texts[index].Font, "|", textPosition + caretPosition, frame.Texts[index].Color);
        }
    }
    ...
    /// <returns>The size and extents of the specified string within the frame</returns>
    ...
    public int GetClosestOpening(string frameName, RectangleF bounds, string text, Vector2 position) 
    {
        ...
        for (int index = 0; index < frame.Texts.Length; ++index) 
        {
            ...
            int openingIndex = this.openingLocator.FindClosestOpening(frame.Texts[index].Font, text, position.X + CaretWidth);
    ...
    {
        this.spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, null, null, this.rasterizerState);
    }
